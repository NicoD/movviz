...
var Repository = function(storage) {
  ...
  /**
   * Callback used when find cursor is ready
   * @callback module:repository/movie~Repository~onFound
   * @param {String} err
   * @param {Object} result - mongodb cursor
   */
   ...
   * @param {rmodule:repository/movie~Repository~onFound}
   */
  this.find = function(criteria, pagination, onFound) {
    var results = collection.find(criteria);
    if(!pagination || !pagination.applyTo) { return onFound(null, results); }
