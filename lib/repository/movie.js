...
var ObjectID = require('mongodb').ObjectID,
  ...
  Repository = function(storage) {
     ...
     * @param {Object} rule - aggregation rule
     * @return {Object} cursor
     * @return {module:repository/movie~Repository~onMapReduced}
     */
    this.getAggregatedResults = function(customlist, callback) {
      ...
      switch(rule.type) {
        case 'map-reduce':
          rule.mapReduce(this, function(err, res) {
            if(err) {
              return callback(err);
            }
            var filter = {};
            ...
            res = res.find(filter).sort(JSON.parse(list.sort));
            callback(err, res);
