...
var CommandRouter = function(argv) {
  ...
  this.get = function(onGet) {
    ...
    MongoClient.connect('mongodb://localhost:27017/movviz', function(err, db) {
      ...
      // a proxy of the callback given to support the close of resources
      var onGetProxy = function(err, controller) {
        if(!err) {
      ...
      switch(program.cmd) {
        case "import":
          if(!program.source) {
            return onGetProxy(new self.CommandError("source not set"));
          }
          fileUtil.prepare(program.source, function(err, uri) {
            if(err) { onGetProxy(err); }

            onGetProxy(
              null,
              require('../action/import').create(uri, movieRepository)
            );
          });
          break;
        ...
        case "list":
          ...
          self.once('criteria-ready', function(criteria) {
            ...
            controller = require('../action/list').create(criteria, movieRepository);
        ...
        case 'install':
          controller = require('../action/install').create(movieRepository);
          onGetProxy(null, controller);

          break;

        default:
          return onGetProxy(new self.CommandError("unknown command '%s'", program.cmd));
      }
    });
  };
};
